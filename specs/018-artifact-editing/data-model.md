...
## Migration Alembic `0019_jdr_artifact_provenance_and_elements_freeform`

**Upgrade.**
1. `add_column` `is_edited` (`Boolean`, server_default `false`, not null), `edited_at` (`DateTime(tz)`, nullable), `edited_by` (`String(64)`, nullable) sur `jdr_artifacts`.
2. Data migration : pour chaque ligne `kind='elements'`, lire `content_json`, aplatir les 4 buckets en `{"elements":[...]}` (correspondance ci-dessus), réécrire. Lignes déjà au format `elements` → inchangées (idempotence).

**Downgrade.**
1. Data migration inverse : regrouper `elements[]` par `category` ; PNJ/Lieux/Objets/Indices → buckets respectifs ; catégories libres → bucket de repli `clues` (best-effort, perte de la catégorie libre assumée et documentée).
2. `drop_column` `edited_by`, `edited_at`, `is_edited`.
