...
2. **Flatten génération** : le job de génération continue de produire 4 buckets ; un helper `elements.flatten_elements(buckets) -> list[dict]` les aplatit avec la correspondance fixe `npcs→"PNJ"`, `locations→"Lieux"`, `items→"Objets"`, `clues→"Indices"` (FR-012). Le helper vit dans `app/services/jdr/elements.py` pour rester partageable par le router/Markdown sans introduire de cycle avec les jobs.
3. **Migration de données** (et non DDL) : la migration `0020` réécrit le `content_json` de chaque ligne `kind='elements'` du même format buckets vers `{"elements":[{category,name,description}, ...]}` via la même correspondance, sans perte (FR-013 / SC-006). Downgrade : regroupe par `category` connue vers buckets, catégories inconnues → bucket de repli documenté (best-effort, perte de catégories libres assumée au downgrade).
...
**Décision.**
1. **Colonnes** (migration `0019`, DDL additif) : `is_edited: bool` (default `false`, not null), `edited_at: datetime|null`, `edited_by: str|null`.
