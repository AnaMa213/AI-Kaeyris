...
async def get_session(
    db: AsyncSession, *, session_id: UUID, gm_key_id: UUID, campaign_id: UUID | None = None
) -> Session | None:
    return await SessionRepository(db).get_for_gm(session_id, gm_key_id, campaign_id)
...
async def create_pj(
    db: AsyncSession, *, name: str, gm_key_id: UUID, campaign_id: UUID | None = None
) -> Pj:
    ...
    try:
        pj = await PjRepository(db).create(
            name=name, owner_gm_key_id=gm_key_id, campaign_id=campaign_id
        )
...
async def enroll_player(
    db: AsyncSession, *, name: str, pj_id: UUID, gm_key_id: UUID, campaign_id: UUID | None = None
) -> EnrollPlayerResult:
