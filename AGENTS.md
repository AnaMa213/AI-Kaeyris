...
> This file is loaded by Codex at every session start. It is the single source of truth for project conventions, principles, and architecture. Treat it as binding.
...
**Primary AI provider**: DeepInfra (cloud), accessed via an agnostic adapter so other providers (Codex, Mistral, Groq, local models) can be plugged in without changing business logic.
...
Deux fichiers dans `docs/`, complémentaires et tenus à jour par Codex :
...
**Règles communes**:
- Ne jamais dupliquer `AGENTS.md` — y faire référence.
...
[`specs/007-identity-refacto-pj-scoping/plan.md`](specs/007-identity-refacto-pj-scoping/plan.md).
